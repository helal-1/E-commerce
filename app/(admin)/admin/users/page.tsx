...
export default function UsersPage() {
    ...
    // دالة جلب البيانات باستخدام useCallback لمنع تكرار الـ Render
    const fetchProfiles = useCallback(async () => {
        try {
            ...
            console.error("خطأ في جلب البيانات:", message);
            return [];
        }
    }, []);
    ...
    useEffect(() => {
        ...
        // نمط التحميل الآمن (Safe Loading Pattern)
    ...
    return (
        <div className="p-4 md:p-8 bg-[#FCFBF9] min-h-screen text-right font-sans" dir="rtl">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col lg:flex-row gap-4 mb-8 font-bold">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pr-12 pl-4 py-4 bg-white border border-[#EDEAE5] rounded-3xl outline-none focus:ring-2 focus:ring-[#8B735B]/20 transition-all shadow-sm text-sm text-[#4A3E31]"
                                ...
                                <tr>
                                    <td colSpan={4} className="p-20 text-center">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <span className="text-[#8B735B] font-serif italic text-sm">جاري تحميل البيانات...</span>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr key={profile.id} className="hover:bg-[#FCFBF9]/50 transition-colors group animate-in fade-in duration-500">
                                    <td className="p-6">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-[#F7F3F0] rounded-full flex items-center justify-center text-[#8B735B] font-black text-sm border border-[#EDEAE5] shadow-sm shrink-0 uppercase">
                                                {profile.full_name ? profile.full_name[0] : <UserCircle size={20} />}
                                            </div>
                    ...
                    {loading ? (
                        ...
                        <div key={profile.id} className="bg-white p-5 rounded-3xl border border-[#EDEAE5] shadow-sm space-y-4 animate-in slide-in-from-bottom-4 duration-500">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 bg-[#F7F3F0] rounded-full flex items-center justify-center text-[#8B735B] font-black text-lg border border-[#EDEAE5] shadow-sm uppercase">
                                        {profile.full_name ? profile.full_name[0] : <UserCircle size={24} />}
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="font-serif text-[#4A3E31] text-lg leading-tight">{profile.full_name || "بدون اسم"}</span>
                            ...
                            <div className="pt-4 border-t border-[#F7F3F0] space-y-2 font-bold">
                                <div className="flex items-center justify-between text-xs">
                                    <span className="text-[#A6998A] uppercase tracking-widest text-[9px]">البريد الإلكتروني</span>
                                    <span className="text-[#4A3E31] italic">{profile.email}</span>
                                </div>
                                <div className="flex items-center justify-between text-xs">
                                    <span className="text-[#A6998A] uppercase tracking-widest text-[9px]">رقم المعرف</span>
                ...
                {filteredUsers.length === 0 && !loading && (
                    <div className="text-center py-24 bg-white rounded-4xl mt-6 border border-dashed border-[#EDEAE5]">
                        <p className="text-[#8B735B] font-serif italic text-sm">لا توجد مقتنيات بشرية مطابقة لبحثك.</p>
                    </div>
                )}
            </div>
        </div>
